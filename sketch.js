...
function preload(){
  // 取得spreadsheet
  jsondata = loadJSON('https://spreadsheets.google.com/feeds/list/1ZOTJqy33u8_e6uCVNRA9H31pwxxTVId6motn6SLiW_k/od6/public/values?alt=json');
}
function setup() {
  ...
  for(let i=0;i<jsondata.feed.entry.length;i+=1){
    str += '<div class="slide-'+i+'">'+
              '<a href="'+ jsondata.feed.entry[i].gsx$link.$t +'" target="_blank">'+
                '<center><img src="./asset/'+temp[i].gsx$img.$t+'" class="img-fluid"><img></center>'+
